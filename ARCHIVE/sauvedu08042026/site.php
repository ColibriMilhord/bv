...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* --- INTRO OVERLAY --- */
        #intro-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: var(--navy-deep); background-image: radial-gradient(circle at center, #0a1128 0%, #050914 100%); z-index: 9999; display: flex; justify-content: center; align-items: center; flex-direction: column; transition: opacity 1.5s ease-in-out, visibility 1.5s; }
        #intro-overlay.hidden { opacity: 0; visibility: hidden; pointer-events: none; }
        .intro-stars-wrapper { display: flex; justify-content: center; gap: 15px; margin-bottom: 30px; }
        .intro-star-svg { width: 30px; height: 30px; fill: url(#goldGradientSvg); opacity: 0; transform: scale(0.5) rotate(-30deg); filter: drop-shadow(0 0 10px rgba(197, 160, 89, 0.3)); }
        .intro-line { height: 2px; width: 100%; max-width: 400px; background: var(--gold-gradient); margin: 0 auto; transform: scaleX(0); opacity: 0; }
        ...
        .intro-main-title { font-family: 'Cinzel', serif; font-size: 3.5rem; font-weight: 700; letter-spacing: 8px; text-transform: uppercase; color: transparent; background: var(--gold-gradient); -webkit-background-clip: text; background-clip: text; opacity: 0; transform: translateY(40px); position: relative; margin: 0; line-height: 1.2; }
        .intro-shimmer { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(120deg, transparent 0%, transparent 40%, rgba(255, 255, 255, 0.6) 50%, transparent 60%, transparent 100%); transform: translateX(-100%); pointer-events: none; }
        .intro-subtitle { font-family: 'Montserrat', sans-serif; font-size: 1.2rem; letter-spacing: 4px; color: var(--gold-text); text-transform: uppercase; margin-top: 20px; opacity: 0; }
        ...
        .animate .intro-star-svg { animation: popStar 1s cubic-bezier(0.175, 0.885, 0.32, 1.275) 2.5s forwards; }
        ...
        @keyframes revealTitle { 0% { transform: translateY(40px); opacity: 0; filter: blur(10px); } 100% { transform: translateY(0); opacity: 1; filter: blur(0px); } }
        @keyframes popStar { 0% { transform: scale(0.5) rotate(-30deg); opacity: 0; } 100% { transform: scale(1) rotate(0deg); opacity: 1; } }
        @keyframes fadeSubtitle { 0% { opacity: 0; letter-spacing: 8px; } 100% { opacity: 1; letter-spacing: 4px; } }
        @keyframes shimmerMove { 0% { transform: translateX(-100%) skewX(-20deg); } 100% { transform: translateX(100%) skewX(-20deg); } }

        /* --- HEADER --- */
        header { position: fixed; top: 0; width: 100%; z-index: 1000; transition: 0.4s ease; padding: 20px 0; background: rgba(5, 9, 20, 0.2); opacity: 0; animation: fadeInHeader 1s ease-out 6.5s forwards; }
        @keyframes fadeInHeader { to { opacity: 1; } }
        header.scrolled { background: rgba(5, 9, 20, 0.98); padding: 10px 0; border-bottom: 1px solid rgba(197, 160, 89, 0.3); box-shadow: 0 5px 20px rgba(0,0,0,0.5); }
        nav { max-width: 1400px; margin: 0 auto; display: flex; justify-content: space-between; align-items: center; padding: 0 5%; }
        .logo { font-size: 1.4rem; color: white; font-weight: 700; display: flex; flex-direction: column; line-height: 1; z-index: 1001; }
        .logo span { font-size: 0.7rem; color: var(--gold-text); font-family: 'Montserrat', sans-serif; letter-spacing: 4px; margin-top: 5px; }
        .nav-links { display: flex; gap: 3rem; list-style: none; }
        .nav-links a { color: rgba(255,255,255,0.9); text-decoration: none; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 1px; transition: 0.3s; position: relative; }
        .nav-links a::after { content: ''; position: absolute; bottom: -5px; left: 0; width: 0; height: 1px; background: var(--gold-text); transition: 0.3s; }
        ...
        .hero-content { position: relative; z-index: 2; text-align: center; color: white; padding: 0 20px; opacity: 0; animation: fadeInHero 2s ease-out 7s forwards; }
        ...
        /* --- SECTION RÉSERVATION --- */
        #reservation { background: #f4f4f4; border-top: 1px solid #ddd; border-bottom: 1px solid #ddd; }
        .booking-layout { display: grid; grid-template-columns: 1fr 1fr; gap: 0; background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.1); }
        .calendar-side { padding: 40px; background: white; border-right: 1px solid #eee; }
        .calendar-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; }
        .month-label { font-family: 'Cinzel', serif; font-size: 1.2rem; color: var(--navy-deep); font-weight: bold; }
        .cal-nav { background: none; border: 1px solid #eee; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; transition: 0.3s; }
        .cal-nav:hover { background: var(--navy-deep); color: white; }
        .days-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 8px; text-align: center; font-size: 0.9rem; }
        .day-label { color: #999; font-size: 0.75rem; margin-bottom: 10px; }
        .day-cell { aspect-ratio: 1; display: flex; align-items: center; justify-content: center; border-radius: 50%; cursor: pointer; transition: 0.2s; font-weight: 500; }
        .day-cell:hover:not(.disabled) { background: #f0f0f0; }
        .day-cell.disabled { color: #ccc; cursor: not-allowed; text-decoration: line-through; }
        .day-cell.selected { background: var(--gold-gradient); color: var(--navy-deep); font-weight: bold; }
        .day-cell.range { background: rgba(197, 160, 89, 0.2); border-radius: 0; }
        .day-cell.range-start { border-radius: 50% 0 0 50%; }
        .day-cell.range-end { border-radius: 0 50% 50% 0; }

        .form-side { padding: 40px; background: #fafafa; }
        .form-title { font-family: 'Cinzel', serif; color: var(--navy-deep); margin-bottom: 20px; font-size: 1.4rem; }
        .summary-box { background: white; padding: 20px; border-left: 3px solid var(--gold-text); margin-bottom: 30px; font-size: 0.9rem; box-shadow: 0 5px 15px rgba(0,0,0,0.03); }
        .lux-input { width: 100%; padding: 12px; margin-bottom: 15px; border: 1px solid #ddd; background: white; border-radius: 4px; font-family: 'Montserrat', sans-serif; }
        .lux-input:focus { border-color: var(--gold-text); outline: none; }
        .option-check { display: flex; align-items: center; gap: 10px; font-size: 0.9rem; margin-bottom: 20px; color: #555; }
        .option-check input { width: 18px; height: 18px; accent-color: var(--gold-text); }
        ...
        .modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 10000; display: flex; justify-content: center; align-items: center; opacity: 0; visibility: hidden; transition: 0.3s; }
        .modal-overlay.active { opacity: 1; visibility: visible; }
        .modal-card { background: white; padding: 50px; border-radius: 8px; text-align: center; max-width: 500px; width: 90%; position: relative; border-top: 5px solid var(--gold-text); }
        .success-icon { width: 60px; height: 60px; background: #e8f5e9; color: #2e7d32; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 30px; margin: 0 auto 20px; }

        @media (max-width: 900px) {
            .hero h1 { font-size: 2.5rem; }
            .experience-grid, .contact-wrapper, .booking-layout { grid-template-columns: 1fr; }
            .calendar-side, .form-side { padding: 20px; }
...
<body class="loading">
    ...
    <header id="navbar">
        <nav>
            ...
            <div class="menu-toggle" onclick="toggleMenu()"><div class="bar"></div><div class="bar"></div><div class="bar"></div></div>
    ...
    <section id="accueil" class="hero">
        <div class="video-background"><iframe src="https://www.youtube.com/embed/19x6z3GPkL4?autoplay=1&mute=1&controls=0&loop=1&playlist=19x6z3GPkL4&playsinline=1&showinfo=0&rel=0&iv_load_policy=3&disablekb=1" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe></div>
    ...
    <section id="experience">
        <div class="section-header"><span class="subtitle">Un lieu unique</span><h2>Entre Luxe & Nature</h2></div>
        <div class="experience-grid">
            <div class="exp-text-block">
                ...
                <div class="stats-row"><div class="stat-item"><h4>200</h4><span>Mètres Carrés</span></div><div class="stat-item"><h4>10</h4><span>Invités</span></div><div class="stat-item"><h4>5</h4><span>Chambres</span></div></div>
            </div>
