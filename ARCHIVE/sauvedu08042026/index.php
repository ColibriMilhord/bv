...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'book') {
    try {
        ...
        $settings_stmt = $pdo->query("SELECT * FROM gite_settings WHERE id = 1");
        $settings      = $settings_stmt ? $settings_stmt->fetch() : [];
        ...
        $client_message = htmlspecialchars($_POST['customer_message'] ?? '');
        $date_debut     = $_POST['check_in'];
        $date_fin       = $_POST['check_out'];
        $option_menage  = isset($_POST['cleaning_fee']) ? 1 : 0;

        if (empty($date_debut) || empty($date_fin)) throw new Exception("Veuillez sélectionner vos dates dans le calendrier.");
        ...
        $to      = "accueil@bellevuedaveyron.com";
        $subject = "Nouvelle demande de réservation - Bellevue d'Aveyron";
        
        // Formatage du message en HTML pour une meilleure lisibilité
        $message_html = "
        <html>
        <head>
            <style>
                body { font-family: 'Montserrat', Arial, sans-serif; color: #333; line-height: 1.6; background-color: #f9f9f9; }
                .container { max-width: 600px; margin: 20px auto; background: #fff; border: 1px solid #e0e0e0; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05); }
                .header { background: #050914; color: #c5a059; padding: 30px; text-align: center; border-bottom: 3px solid #bf953f; }
                .header h2 { margin: 0; text-transform: uppercase; letter-spacing: 2px; font-size: 20px; }
                .content { padding: 30px; }
                .section-title { color: #bf953f; font-weight: bold; text-transform: uppercase; font-size: 14px; border-bottom: 1px solid #eee; padding-bottom: 8px; margin: 20px 0 15px; }
                .item { margin-bottom: 12px; display: flex; }
                .label { font-weight: 600; width: 140px; color: #555; }
                .value { flex: 1; color: #222; }
                .message-box { background: #fcf6ba33; border-left: 3px solid #bf953f; padding: 15px; font-style: italic; margin-top: 10px; border-radius: 0 4px 4px 0; }
                .footer { background: #f8f8f8; padding: 20px; font-size: 12px; color: #888; text-align: center; border-top: 1px solid #eee; }
                .btn-reply { display: inline-block; background: #bf953f; color: #fff; padding: 12px 25px; text-decoration: none; border-radius: 6px; margin-top: 20px; font-weight: bold; }
            </style>
        </head>
        <body>
            <div class='container'>
                <div class='header'>
                    ...
                    <div style='font-size: 12px; margin-top: 5px; opacity: 0.8;'>Bellevue d'Aveyron - Villa 5 ★</div>
                </div>
                <div class='content'>
                    <div class='section-title'>Coordonnées du Client</div>
                    <div class='item'><span class='label'>Nom :</span> <span class='value'>$client_nom</span></div>
                    <div class='item'><span class='label'>E-mail :</span> <span class='value'>$client_email</span></div>
                    <div class='item'><span class='label'>Téléphone :</span> <span class='value'>$client_tel</span></div>
                    
                    <div class='section-title'>Détails du Séjour</div>
                    <div class='item'><span class='label'>Arrivée :</span> <span class='value'>" . date('d/m/Y', strtotime($date_debut)) . "</span></div>
                    <div class='item'><span class='label'>Départ :</span> <span class='value'>" . date('d/m/Y', strtotime($date_fin)) . "</span></div>
                    <div class='item'><span class='label'>Durée :</span> <span class='value'>$nuits nuits</span></div>
                    <div class='item'><span class='label'>Option Ménage :</span> <span class='value'>" . ($option_menage ? 'Oui' : 'Non') . "</span></div>
                    <div class='item'><span class='label'>Montant Total :</span> <span class='value' style='font-weight:bold; color:#050914;'>" . number_format($prix_total, 2, ',', ' ') . " €</span></div>
                    <div class='item'><span class='label'>Acompte (30%) :</span> <span class='value'>" . number_format($acompte_montant, 2, ',', ' ') . " €</span></div>
                    
                    <div class='section-title'>Message du Client</div>
                    <div class='message-box'>" . (!empty($client_message) ? nl2br($client_message) : "Aucun message particulier.") . "</div>
                    
                    <div style='text-align: center;'>
                        <a href='mailto:$client_email' class='btn-reply'>Répondre au client</a>
                    </div>
                </div>
                <div class='footer'>
                    Cet email a été envoyé automatiquement depuis le site <a href='https://bellevuedaveyron.com' style='color:#bf953f;'>bellevuedaveyron.com</a>.<br>
                    Pour répondre au client, cliquez simplement sur 'Répondre' dans votre logiciel de messagerie.
                </div>
            </div>
        </body>
        </html>
...
<body class="loading">
    ...
    <!-- ══════════════════════════════════════════
         ...
         ══════════════════════════════════════════ -->
    <script>
        // Masquage immédiat (avant le rendu) si :
        // 1. Déjà joué dans la session
        // 2. Succès d'une réservation (variable PHP)
        // 3. Accès direct via une ancre (#services, #tarifs, etc.)
            ...
            window.addEventListener('load', function () {
                ...
                if (overlay) {
                    if (isSuccess || introPlayed || hasHash) {
                        ...
                        if (isSuccess && history.replaceState) {
